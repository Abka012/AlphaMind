...
def add_features(df):
    ...
    df["tick_position"] = (df["close"] - df["tick_low_100"]) / (
        df["tick_high_100"] - df["tick_low_100"] + 1e-9
    )
    df["tick_position_normalized"] = (df["close"] - df["tick_ma_50"]) / (
        df["tick_std"] + 1e-9
    )
    ...
    # Regime features
    df["tick_vol_regime"] = (
        df["tick_std"] > df["tick_std"].rolling(200, min_periods=1).mean()
    ).astype(int)
    ...
    df["vwap"] = (df["close"] * df["tick_volume"]).rolling(
        window=50, min_periods=1
    ).sum() / (df["tick_volume"].rolling(window=50, min_periods=1).sum() + 1e-9)
    df["vwap_deviation"] = (df["close"] - df["vwap"]) / (df["tick_std"] + 1e-9)

    df["volume_weighted_imbalance"] = (df["bidVolume"] - df["askVolume"]).rolling(
...
def add_targets(df, horizons=[50, 100, 200]):
    """Predict: will price move profitably in next N ticks for multiple horizons?"""
    print(f"Computing targets (horizons={horizons} ticks)...")

    df = df.copy()

    for horizon in horizons:
        # Future return after horizon ticks (percentage-based)
        future_return = df["close"].shift(-horizon) / df["close"] - 1

        df[f"target_return_{horizon}"] = future_return

        # Direction: price goes up = 1, down = 0
        df[f"target_direction_{horizon}"] = (future_return > 0).astype(int)

        # Opportunity: significant move threshold (0.02%)
        min_profit_pct = 0.0002
        df[f"target_opportunity_{horizon}"] = (
            future_return.abs() > min_profit_pct
        ).astype(int)

    # Legacy support (using the largest horizon as default)
    max_h = max(horizons)
    df["target_return"] = df[f"target_return_{max_h}"]
    df["target_direction"] = df[f"target_direction_{max_h}"]
    df["target_opportunity"] = df[f"target_opportunity_{max_h}"]

    # Drop rows where any target is NaN (due to shifting)
    target_cols = [f"target_direction_{h}" for h in horizons] + [
        f"target_opportunity_{h}" for h in horizons
    ]
